...
procedure minimo(var v:v_detalles; var v_reg:v_reg; var min:ped; var n:integer);
var 	
	i:integer;
    m:integer;
    begin
m:=999999;
for i:=1 to 4 do
	if (v_reg[i].cod<m) then begin
    m:=v_reg[i].cod;
	n:=i;
    min:=v_reg[i];
  end;
leerDetalle(v[n],v_reg[n]);
end;

procedure actualizarMaestro(var arch_m:arch_m;var vec_d:v_detalles);
var
v_re:v_reg;
i:integer;
aux2:e;
...
begin
    ...
    minimo(vec_d,v_re, min, indice);
    leerMaestro(arch_m, aux2);
    while(min.cod<>valor_alto) and not(eof(arch_m)) do begin
        if (aux2.cod<>min.cod) then
    	   while (aux2.cod<min.cod)do
                   leerMaestro(arch_m, aux2);
		diferencia:=aux2.stock_a-min.cant;
        if(diferencia < aux2.stock_m) then
				writeln(' le cant del prod', aux2.cod, ' quedo por debajo del stock min');
        if(diferencia<0) then begin
			writeln(' faltan ',(diferencia*(-1)),' de cant del prod', aux2.cod, ' de la sucursal ',indice);
			diferencia:=0;
		end;
